...
def encode_tf(labels, priors, match_thresh, ignore_thresh,
    ...
    matches_smile_v = tf.gather(smile_valid, best_truth_idx)  # [num_priors]

    loc_t = _encode_bbox(matches_bbox, priors, variances)
    smile_valid_t = tf.cast(matches_smile_v > 0, tf.float32)
    ...
    return tf.concat([loc_t, smile_valid_t[..., tf.newaxis],
...
def decode_tf(labels, priors, variances=[0.1, 0.2]):
    ...
    smile_valid = labels[:, 4][:, tf.newaxis]
    conf = labels[:, 5][:, tf.newaxis]

    return tf.concat([bbox, smile_valid, conf], axis=1)
