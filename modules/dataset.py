...
from modules.anchor import encode_tf, prior_box, _decode_bbox
...
def _parse_tfrecord(img_dim, using_bin, using_flip, using_distort,
    ...
    def parse_tfrecord(tfrecord):
        features = {
            ...
            'image/object/smile/valid': tf.io.VarLenFeature(tf.float32)}
        if using_bin:
        ...
        labels = tf.stack(
            [tf.sparse.to_dense(x['image/object/bbox/xmin']),
             ...
             tf.sparse.to_dense(x['image/object/smile/valid'])], axis=1)
...
def _crop(img, labels, max_loop=250):
    ...
    def matrix_iof(a, b):
        ...
        area_a = tf.cast(area_a, tf.float32)
...
if __name__ == '__main__':
    ...
    img_path = '/Users/lichaochao/Downloads/CelebA/CelebA/test/200001.jpg'
    img_raw = cv2.imread(img_path)
    label = [680, 536, 686, 950, 1]
    label[2] += label[0]
    label[3] += label[1]
    print(label)
    labels = [label]
    ...
    for i in range(labels.shape[0]):
        if labels[i][-1] > 0:
            ...
            x2 = int(box[0] * img_w + box[2] * img_w / 2)
            y2 = int(box[1] * img_h + box[3] * img_h / 2)
